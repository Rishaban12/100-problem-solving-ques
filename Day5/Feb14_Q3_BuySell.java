/* 3. You are given an array prices where prices[i] is the price of a given stock on the ith day. You want to
...
public static void main(String[] args){
   int[] arr={7,6,4,3,1};
   int buy=arr[0],sell=0,max=0;
   for(int i=1;i<arr.length;i++){
    if(arr[i]<buy){
       buy=arr[i];
     }else if(buy!=0 && buy<arr[i]){
      sell=arr[i]-buy;
